...
export class AppOne {
    ...
    createScene(): BABYLON.Scene {
        ...
        scene.enablePhysics(undefined, new BABYLON.CannonJSPlugin(undefined, undefined, CANNON));


        // const torus = BABYLON.MeshBuilder.CreateTorus("torus", { diameter: 0.5, thickness: 0.2, tessellation: 50 }, scene);
        const torus = BABYLON.MeshBuilder.CreateSphere("target", { diameter: 0.5 }, scene);
        torus.position.y = 2;
        torus.material = this.mats.grayColor();
        torus.physicsImpostor = new BABYLON.PhysicsImpostor(torus, BABYLON.PhysicsImpostor.SphereImpostor, { mass: 1, restitution: 0.5 }, scene)

        // Our built-in 'ground' shape.
        const table = BABYLON.MeshBuilder.CreateGround("ground", { width: 6, height: 2 }, scene);
        table.position.y = 1;
        table.physicsImpostor = new BABYLON.PhysicsImpostor(table, BABYLON.PhysicsImpostor.BoxImpostor, { mass: 0 })

        return scene;
    }
    initFreeCam(canvas: HTMLCanvasElement) {
        console.log("Enable Free Camera")
        var camera = new BABYLON.FreeCamera("camera1", new BABYLON.Vector3(0, 2.5, -6), this.scene);
